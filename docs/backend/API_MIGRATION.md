...
Status note:
- Non-chat migration plan is still active.
- Matrix event-room mapping endpoint is still pending backend implementation.
- Use `docs/chat/MATRIX_STATUS.md` as the live chat delivery status.
...
Related document:
- Mobile chat status and plan: `docs/chat/MATRIX_STATUS.md`
...
Chat capabilities move to Matrix APIs and SDK behavior, matching Element-style patterns documented in `docs/chat/MATRIX_STATUS.md`.
...
- Chat UI should follow Element interaction and visual patterns (room list, timeline, composer, action list), as mapped in `docs/chat/MATRIX_STATUS.md`.
...
- Kotlin app chat features must migrate to Matrix SDK surface (per `docs/chat/MATRIX_STATUS.md`), not `ApiService` chat endpoints.
...
- Migrate mobile chat stack to Matrix SDK abstractions from `docs/chat/MATRIX_STATUS.md`.
- Implement Element-aligned chat UI/UX patterns from `docs/chat/MATRIX_STATUS.md` design references.
- Disable legacy chat endpoints in Rust (`/api/v1/chats`, `/ws/chat`) with `410 Gone`.

Exit criteria:
- Chat UX works end-to-end via Matrix.
- Chat UI reaches MVP design parity checks defined in `docs/chat/MATRIX_STATUS.md`.
