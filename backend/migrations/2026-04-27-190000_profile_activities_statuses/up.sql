...
ALTER TABLE public.profiles
    ...
    CHECK (status_text IS NULL OR char_length(status_text) <= 160);

-- Refresh generated search vectors so public status participates in profile search.
...
FROM (VALUES
    ('7b01dd2e-01ff-501e-bba5-e75f8ee3d4fa', 'Nauka'),
    ('c7104ef3-a7fe-5eb0-b0eb-542c1f46618a', 'Sport'),
    ('89f5603c-9974-5e2c-8b76-21097f40e6b0', 'Siłownia'),
    ('45d64e17-4069-5db5-a4a4-8f6c3aa79908', 'Bieganie'),
    ('e63fb3e4-a029-5ae2-b33c-9e5b99f765d2', 'Rower'),
    ('934d8002-cea6-524f-aab0-dae497fbf067', 'Planszówki'),
    ('da79350b-bf17-5c7b-8d2f-7294229f53f4', 'Kawa'),
    ('77ed4d70-4523-525e-aced-e2498dda21b3', 'Spacer'),
    ('aac2fb77-2ab8-5e47-b930-1610ba7cb462', 'Koncerty'),
    ('e83e83b7-1a35-503f-b8e6-1355afc16ff0', 'Gotowanie'),
    ('2b42b1c9-35ae-50a8-8d43-7487f67684c5', 'Wolontariat'),
    ('19889079-09b1-5a5e-981d-d8ff5000b4f8', 'Programowanie'),
    ('869196ff-32cf-57d3-9997-a06070b5cfbb', 'Film'),
    ('05126d48-de28-5e2c-a9a5-a78934b7dd1c', 'Czytanie')
) AS seed(id, name)
WHERE NOT EXISTS (
    SELECT 1 FROM public.tags t
    WHERE t.scope = 'activity' AND lower(t.name) = lower(seed.name)
);
