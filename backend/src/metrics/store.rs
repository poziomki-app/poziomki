...
impl TimeSeries {
    ...
    pub fn push(&self, timestamp_epoch: u32, value: f32) {
        ...
        if let (Some(ts_slot), Some(val_slot)) = (self.timestamps.get(head), self.values.get(head))
        {
    ...
    pub fn read_range(&self, from: u32, to: u32) -> (Vec<u32>, Vec<f32>) {
        ...
        for i in 0..count {
            ...
            let (Some(ts_slot), Some(val_slot)) = (self.timestamps.get(idx), self.values.get(idx))
            else {
                continue;
            };
