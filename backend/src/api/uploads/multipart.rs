...
pub(in crate::api) type HandlerError = Box<Response>;
pub(in crate::api) type HandlerResult<T> = std::result::Result<T, HandlerError>;

pub(in crate::api) struct ParsedUpload {
    pub(in crate::api) context: UploadContext,
    pub(in crate::api) context_id: Option<String>,
    pub(in crate::api) mime_type: String,
    pub(in crate::api) bytes: Vec<u8>,
}
...
pub(in crate::api) async fn read_multipart(
    headers: &HeaderMap,
