...
mod tests {
    ...
    fn make_event_model(lat: Option<f64>, lng: Option<f64>) -> events::Model {
        events::Model {
            id: Uuid::new_v4(),
    ...
    #[test]
    fn geo_from_event_both_present() {
        let event = make_event_model(Some(52.2297), Some(21.0122));
        let geo = geo_from_event(&event).expect("should return Some");
        assert!((geo.lat - 52.2297).abs() < f64::EPSILON);
        assert!((geo.lng - 21.0122).abs() < f64::EPSILON);
    }

    #[test]
    fn geo_from_event_both_none() {
        let event = make_event_model(None, None);
        assert!(geo_from_event(&event).is_none());
    }

    #[test]
    fn geo_from_event_only_lat() {
        let event = make_event_model(Some(52.0), None);
        assert!(
            geo_from_event(&event).is_none(),
            "should be None when longitude is missing"
        );
    }

    #[test]
    fn geo_from_event_only_lng() {
        let event = make_event_model(None, Some(21.0));
        assert!(
            geo_from_event(&event).is_none(),
            "should be None when latitude is missing"
        );
    }

    #[test]
    fn geo_from_event_zero_coords_is_some() {
        let event = make_event_model(Some(0.0), Some(0.0));
        let geo = geo_from_event(&event).expect("zero coords should still be Some");
        assert!((geo.lat).abs() < f64::EPSILON);
        assert!((geo.lng).abs() < f64::EPSILON);
    }

    #[test]
    fn geo_from_event_negative_coords() {
        let event = make_event_model(Some(-33.8688), Some(-151.2093));
        let geo = geo_from_event(&event).expect("negative coords should work");
        assert!((geo.lat - (-33.8688)).abs() < f64::EPSILON);
        assert!((geo.lng - (-151.2093)).abs() < f64::EPSILON);
    }
}
