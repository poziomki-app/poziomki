...
pub(in crate::controllers::migration_api) struct ImageVariants {
    pub(in crate::controllers::migration_api) thumbnail: Vec<u8>,
    pub(in crate::controllers::migration_api) standard: Vec<u8>,
    pub(in crate::controllers::migration_api) thumbhash: Vec<u8>,
}

/// Async entry point — acquires semaphore and offloads CPU work.
pub(in crate::controllers::migration_api) async fn generate_variants(
    bytes: &[u8],
    mime: &str,
...
fn generate_variants_blocking(bytes: &[u8], mime: &str) -> Result<ImageVariants, String> {
    ...
    let src_image = Image::from_vec_u8(orig_w, orig_h, rgba.into_raw(), PixelType::U8x4)
        .map_err(|e| format!("src image: {e}"))?;

    // Thumbnail: max 200px
    let thumbnail = resize_and_encode_webp(&src_image, orig_w, orig_h, 200, 75.0)?;

    // Standard: max 800px
    let standard = resize_and_encode_webp(&src_image, orig_w, orig_h, 800, 80.0)?;
...
fn resize_and_encode_webp(
    ...
    let (dst_w, dst_h) = fit_dimensions(orig_w, orig_h, max_dim);
    let resized = resize_rgba(src, orig_w, orig_h, dst_w, dst_h)?;
...
fn compute_thumbhash(src: &Image<'_>, orig_w: u32, orig_h: u32) -> Result<Vec<u8>, String> {
    ...
    let resized = resize_rgba(src, orig_w, orig_h, tw, th)?;
...
pub(in crate::controllers::migration_api) fn variant_filename(
    original: &str,
    suffix: &str,
) -> String {
    let stem = original
