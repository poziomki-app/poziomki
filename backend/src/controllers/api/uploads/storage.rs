...
pub(in crate::controllers::api) enum StorageErrorKind {
    NotFound,
}

#[derive(Clone, Debug)]
pub(in crate::controllers::api) struct StorageError {
    pub(in crate::controllers::api) kind: Option<StorageErrorKind>,
}
...
pub(in crate::controllers::api) async fn read(filename: &str) -> Result<Vec<u8>, StorageError> {
    let config = storage().map_err(|_message| storage_error(None))?;
    let response = config
        .bucket
...
pub(in crate::controllers::api) async fn exists(filename: &str) -> Result<bool, StorageError> {
    let config = storage().map_err(|_message| storage_error(None))?;
...
pub(in crate::controllers::api) async fn signed_get_url(
    filename: &str,
...
pub(in crate::controllers::api) async fn signed_put_url(
    filename: &str,
    mime_type: &str,
) -> Result<String, StorageError> {
    let config = storage().map_err(|_message| storage_error(None))?;
