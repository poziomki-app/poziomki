...
pub(in crate::controllers::api) struct ImageVariants {
    pub(in crate::controllers::api) thumbnail: Vec<u8>,
    pub(in crate::controllers::api) standard: Vec<u8>,
    pub(in crate::controllers::api) thumbhash: Vec<u8>,
}

/// Async entry point — acquires semaphore and offloads CPU work.
pub(in crate::controllers::api) async fn generate_variants(
    bytes: &[u8],
    mime: &str,
...
pub(in crate::controllers::api) fn variant_filename(original: &str, suffix: &str) -> String {
    let stem = original
        .rfind('.')
